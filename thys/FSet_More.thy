...
lemma fMax_finsert[simp]: "A \<noteq> {||} \<Longrightarrow> fMax (finsert x A) = max x (fMax A)"
  by transfer (rule Max_insert)

lemma fMin_finsert[simp]: "A \<noteq> {||} \<Longrightarrow> fMin (finsert x A) = min x (fMin A)"
  by transfer (rule Min_insert)
